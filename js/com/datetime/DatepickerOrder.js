(function($){
	 ...
	 $.fn.initDate=function(options){
		...
		$(this).click(function(){
			...
			var readSourceDate=_dtime.vars.sourceDate=[];
			_dtime.vars.publicDate=[];
			var readjson=eval('('+$(opts.newdate).val()+')');
			var readsourcejson=eval('('+$(opts.sourcedate).val()+')');
			$.each(readjson,function(i,p){
				readDate.push(_cutsign(p,'-','/'));
				_dtime.vars.publicDate.push(_cutsign(p,'-','/'));
			});
			$.each(readsourcejson,function(i,p){
				readSourceDate.push(_cutsign(p,'-','/'));
			});
			_exeAttach.loadTd(opts,readDate,readSourceDate);
			return false;
		});
	}
	/*---public json---*/
	...
	_exeAttach={
		ds:function(_this,opts){
			...
			rDateStr.push(']');
			$(opts.newdate).empty().val(rDateStr.join(''));
			$('.date-area').hide();
		},
		dr:function(opts){
			var readDate=_dtime.vars.readDate=[];
			...
			this.loadTd(opts,readDate,readSourceDate);
		...
		prev:function($lObj,$rObj,opts){
			...
			if(_lm==-1){_lm=11;_ly--}
			if(_rm==-1){_rm=11;_ry--}
			_loadMonth($lObj,opts,_dtime.vars.readDate,_ly,_lm,_dtime.vars.sourceDate);
			_loadMonth($rObj,opts,_dtime.vars.readDate,_ry,_rm,_dtime.vars.sourceDate);
		},
		...
		loadTd:function(opts,readDate,readSourceDate){
			if($(opts.sourcedate).val()!=''){
				...
				var $lObj=$dateAreaObj.find('table').eq(0);
				var $rObj=$dateAreaObj.find('table').eq(1);
				_loadMonth($lObj,opts,readDate,_first.y,_first.m,readSourceDate);
				_loadMonth($rObj,opts,readDate,_oy,_om,readSourceDate);
			}		
		},
		addsed:function($td,_get){
			if(!$td.hasClass(_dtime.css.past)){
				$td.attr('class','');
				$td.addClass(_dtime.css.selected);
			}
			if($.inArray(_get,_dtime.vars.sourceDate)!=-1){
		...
		slimit:function(othis,opts){
			...
			case 0:
				...
				$this.loadTd(opts,_dtime.vars.readDate,_dtime.vars.sourceDate);
			break;
			case 1:
				_dtime.vars.readDate=[];
				jQuery.map(_dtime.vars.publicDate,function(n){
				...
				if(_array!=''){$this.loadTd(opts,_array,_dtime.vars.sourceDate);}else{$this.emptysign();}
	...
	/*---load month---*/
	_loadMonth=function(_obj,_opts,_readDate,_y,_m,_source){
		var date=[];
		...
		for(var i=0;i<_days;i++){
			...
			var _curymd=_returnFormat(new Date(_y,_m,i+1),'/');
			if($.inArray(_curymd,_source)==-1){
				...
				if(new Date(_alt).getTime()>new Date(_first).getTime()){  //after
					...
					for(var i=0;i<_days;i++){
						...
						if(_cur<=new Date(_alt)&&_cur>=new Date(_first)){
							...
							if($td.text()!=''&&(!$td.hasClass(_dtime.css.past))){
								$td.attr('class','');
								if(i==_days-1){
